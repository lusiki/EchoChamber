...
```{r}
### Figure 1: Community Composition Bar Chart (Revised)

# This figure provides the primary evidence for structural segregation along subreddit lines.
# We will group smaller subreddits into an "Other" category to make the plot readable.
...
ggplot(plot_data_grouped, aes(x = Louvain_Community, y = Member_Count, fill = subreddit_group)) +
  geom_bar(stat = "identity", position = "stack") +
  theme_minimal(base_family = "sans") +
  scale_fill_viridis_d(option = "cividis", name = "Primary Subreddit") +
  labs(
...
ggplot(connector_df, aes(x = P)) +
  ...
  labs(
    title = "Distribution of User Bridge Scores (Participation Coefficient, P)",
    subtitle = "The median user is a 'bridge' (P > 0.5), indicating high cross-community engagement.",
...
ggplot(sentiment_plot_data, aes(x = as.factor(community), y = proportion, fill = AUTO_SENTIMENT)) +
  ...
  theme_minimal(base_family = "sans") +
  labs(
    title = "Comparative Sentiment Analysis of Top Network Communities",
...
ggraph(tg_for_plotting, layout = "fr") +
  ...
  theme_graph(base_family = "sans") +
  labs(
    title = "The 'Clash of Communities' Network: Main Component",
    subtitle = "Node size & color are proportional to the user's bridge score (P).",
